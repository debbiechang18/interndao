...
function Nav() {
  ...
  return (
    <div className={`nav-container nav-container${isHomePage ? '-home' : ''}`}>
      <Link className={`desktop-logo${isHomePage ? '-home' : ''}`} to="/">
        <GetLogo logoType={`${isHomePage ? 'desktopLogoHome' : 'desktopLogo'}`} loading="lazy" />
      </Link>
